function [vco,Gpoly1] = getpolyRegCutoff(p)
...
%     hold on;
%     plot(vaxis1,GpolyLog1,'y-','LineWidth',2);
%     xlim([-10 10]);
%     ylim([-80 5]);
%     hold off;
...
%     
    for k = 1:1
        eval([ '[mini, ix] = min(abs(GpolyLog' num2str(k) '+6));'])
        eval(['vco(k) = abs(vaxis' num2str(k) '(ix))/100;']);
    end

function [finalEsts, corrValtot, chosenAliasPat] = blockMatching( iqhpComp, vCand, ...
    ...
    %% Find correlation for all candidate velocity vectors using compounded data (GPU)   
        ...
        corrVal = gpuArray(zeros( size( vCand, 3), nrpoints, lowResPacketSize-1));
        gData = gpuArray(iqhpComp);
        gAxisX = gpuArray(bfPars.x_axis);
        gAxisZ = gpuArray(bfPars.z_axis);

        coords_origx = gpuArray( -(kernelsize(1)-1)/2*dx:dx:(kernelsize(1)-1)/2*dx );
        coords_origz = gpuArray( -(kernelsize(2)-1)/2*dz:dz:(kernelsize(2)-1)/2*dz );
